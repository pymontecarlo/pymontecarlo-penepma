...
class InteractionForcings(base.OptionBase):
    ...
    def __init__(self, bremsstrahlung_xray_splitting_factor=1, characteristic_xray_splitting_factor=1):
        super().__init__()
    ...
    def __iter__(self):
        for (particle, collision), (forcer, weight_low, weight_high) in self._interactions.items():
            yield particle, collision, forcer, weight_low, weight_high

    def __eq__(self, other):
        return super().__eq__(other) and \
            base.are_mapping_value_close(self._interactions, other._interactions, abs_tol=self.TOLERANCE) and \
            base.isclose(self.bremsstrahlung_xray_splitting_factor, other.bremsstrahlung_xray_splitting_factor) and \
            base.isclose(self.characteristic_xray_splitting_factor, other.characteristic_xray_splitting_factor)

    def add(self, particle, collision, forcer, weight_low, weight_high):
        key = (particle, collision)
        if key in self._interactions:
            raise ValueError('Interaction forcing for particle {} and collision {} already exists'
                             .format(particle, collision))
...
#region HDF5
    ...
    def parse_hdf5(cls, group):
        bremsstrahlung_xray_splitting_factor = cls._parse_hdf5(group, cls.ATTR_BREMSSTRAHLUNG_XRAY_SPLITTING_FACTOR, int)
        characteristic_xray_splitting_factor = cls._parse_hdf5(group, cls.ATTR_CHARACTERISTIC_XRAY_SPLITTING_FACTOR, int)
        interactions = cls(bremsstrahlung_xray_splitting_factor, characteristic_xray_splitting_factor)
        ...
        for particle, collision, forcer, weight_low, weight_high in \
                zip(particles, collisions, forcers, weights_low, weights_high):
    ...
    def convert_hdf5(self, group):
        ...
        self._convert_hdf5(group, self.ATTR_BREMSSTRAHLUNG_XRAY_SPLITTING_FACTOR, self.bremsstrahlung_xray_splitting_factor)
        self._convert_hdf5(group, self.ATTR_CHARACTERISTIC_XRAY_SPLITTING_FACTOR, self.characteristic_xray_splitting_factor)
        ...
        dtype = np.float
        dataset_weight_high = group.create_dataset(self.DATASET_WEIGHT_HIGH, shape, dtype)

        for i, (particle, collision, forcer, weight_low, weight_high) in enumerate(self):
...
#region Series
    ...
    def convert_series(self, builder):
        ...
        builder.add_column('bremsstrahlung xray splitting factor', 'bremms splitting', self.bremsstrahlung_xray_splitting_factor)
        builder.add_column('characteristic xray splitting factor', 'charac splitting', self.bremsstrahlung_xray_splitting_factor)
...
#region Document
    ...
    def convert_document(self, builder):
        ...
        description.add_item('Bremmstrahlung X-ray splitting factor', self.bremsstrahlung_xray_splitting_factor)
        description.add_item('Characteristic X-ray splitting factor', self.characteristic_xray_splitting_factor)
...
class LazyInteractionForcings(base.LazyOptionBase):
    ...
    def apply(self, parent_option, options):
        ...
        if options.beam.particle == Particle.ELECTRON:
            interactions.add(Particle.ELECTRON, ICOL.HARD_BREMSSTRAHLUNG_EMISSION, -40, 1e-4, 1.0)
            interactions.add(Particle.ELECTRON, ICOL.INNER_SHELL_IMPACT_IONISATION, -40, 1e-4, 1.0)
