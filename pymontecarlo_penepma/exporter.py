...
from pypenelopetools.material import Material as PenelopeMaterial, VACUUM as PENELOPE_VACUUM
...
PARTICLE_INDEX = {Particle.ELECTRON: KPAR.ELECTRON,
...
class PenepmaExporter(ExporterBase):
    ...
    async def _write_material(self, penmaterial, options, dirpath, erracc):
        ...
        with tempfile.TemporaryFile('w+') as fileobj:
            ...
            if returncode != 0:
                exc = RuntimeError('Error creating material {}'.format(penmaterial.name))
                erracc.add_exception(exc)

        # Move material file to output directory
        srcfilepath = os.path.join(options.program.pendbase_directory, penmaterial.filename)
    ...
    def _export_material(self, material, options, erracc, index):
        ...
        penmaterial = PenelopeMaterial(name, composition, density_g_per_cm3,
                                       filename=filename)
    ...
    def _validate_program(self, program, options, erracc):
        ...
        simulation_parameters = apply_lazy(program.simulation_parameters, program, options)

        if simulation_parameters.eabs_electron_eV < 0.0:
            exc = ValueError('Electron absorption energy must be greater or equal to 0.0')
            erracc.add_exception(exc)
        ...
        if simulation_parameters.eabs_positron_eV < 0.0:
            exc = ValueError('Positron absorption energy must be greater or equal to 0.0')
            erracc.add_exception(exc)
        ...
        # equal to 50.0 eV.
        interaction_forcings = apply_lazy(program.interaction_forcings, program, options)

        for particle, collision, forcer, weight_low, weight_high in interaction_forcings:
            ...
            if abs(forcer) < 1:
                exc = ValueError('Absolute value of forcer must be greater or equal to 1')
                erracc.add_exception(exc)
        ...
        if interaction_forcings.bremsstrahlung_xray_splitting_factor < 1:
            exc = ValueError('Bremsstrahlung X-ray splitting factor must be greater or equal to 1')
            erracc.add_exception(exc)

        if interaction_forcings.characteristic_xray_splitting_factor < 1:
            exc = ValueError('Characteristic X-ray splitting factor must be greater or equal to 1')
            erracc.add_exception(exc)
    ...
    def _export_program(self, program, options, erracc, input, geometry, index_lookup):
        ...
        for penmaterial in geometry.get_materials():
            ...
            input.materials.add(index, filename,
        ...
        for module in geometry.get_modules():
            ...
            for particle, collision, forcer, weight_low, weight_high in interaction_forcings:
                ...
                input.IFORCE.add(index, particle, collision, forcer, weight_low, weight_high)

            if interaction_forcings.bremsstrahlung_xray_splitting_factor > 1:
                input.IBRSPL.add(index, interaction_forcings.bremsstrahlung_xray_splitting_factor)

            if interaction_forcings.characteristic_xray_splitting_factor > 1:
                input.IXRSPL.add(index, interaction_forcings.characteristic_xray_splitting_factor)
    ...
    def _validate_beam(self, beam, options, erracc):
        ...
        if energy_eV < 100:
            exc = ValueError('Energy ({} eV) must be greater than 100 eV'
                             .format(energy_eV))
            erracc.add_exception(exc)

        if energy_eV > 1e9:
            exc = ValueError('Energy ({} eV) must be less than 1 GeV'
                             .format(energy_eV))
            erracc.add_exception(exc)
        ...
        if particle not in PARTICLE_INDEX:
            exc = ValueError('Particle ({}) is not available'
                             .format(particle))
            erracc.add_exception(exc)
    ...
    def _validate_sample(self, sample, options, erracc):
        ...
        if len(options.sample.materials) > MAXIMUM_NUMBER_MATERIALS:
            exc = ValueError("PENEPMA supports a maximum of {} materials."
    ...
    def _export_sample_horizontallayers(self, sample, options, erracc, geometry, dsmaxs):
        ...
        index = 0
        tmpgrouping = []
        for layer, (surface_top, surface_bottom) in zip(layers, _pairwise(surface_layers)):
        ...
        if sample.has_substrate():
            ...
            surface_bottom = zplane(surface_top.shift.z_cm - 100) # 100 cm below last layer

            substrate_material = apply_lazy(sample.substrate_material, sample, options)
            penmaterial = self._export_material(substrate_material, options, erracc, index)
    ...
    def _export_sample_verticallayers(self, sample, options, erracc, geometry, dsmaxs):
        ...
        ## Layers
        for layer, (surface_left, surface_right) in zip(layers, _pairwise(surface_layers)):
            material = apply_lazy(layer.material, layer, options)
            penmaterial = self._export_material(material, options, erracc, index)
    ...
    def _validate_detector(self, detector, options, erracc):
        ...
        if len(photon_detectors) > MAXIMUM_NUMBER_PHOTON_DETECTORS:
            exc = ValueError('PENEPMA only supports up to {} photon detectors. {} are defined'
                              .format(MAXIMUM_NUMBER_PHOTON_DETECTORS, len(photon_detectors)))
            erracc.add_exception(exc)
